...
if (isset($_POST['versPersonne'])) {
    header('location:modifierEditionPersonne.php');
    die();
}
...
if (isset($_POST['modifierEdition'])) {
    ...
    if (isset($_SESSION['edition'])) {
        ...
        if (isset($_POST['nomEdition']) && isset($_POST['dureeEdition']) && isset($_POST['dateEdition'])) {
            ...
            if ($nomEdition == "" || $dureeEdition == "" || $dateEdition == "") {
                /* erreur si au moins un des champs n'est pas remplis */
                $erreur = true;
                $_SESSION['error'] = "Tous les champs doivent être remplis.";
            } else if (!preg_match("/^[0-9]*$/", $dureeEdition)) {
                /* erreur si la durée n'est pas un nombre */
                $erreur = true;
                $_SESSION['error'] = "La durée de l'édition doit être un nombre.";
            } else {
                /* pas d'erreur donc requête de sélection de l'édition */
        ...
        if ($erreur == false) {
            /* si aucune erreur n'est détectée : enregistrement */
            ...
            if ($result3 == true) {
                /* si la requête a fonctionné : mise à jour de la table exister */
                $requete4 = "UPDATE exister SET idEdition='" . $nomEdition . "' WHERE idEdition='" . $ancienneEdition . "'";
                $result4 = mysqli_query($link, $requete4);

                if ($result4 == true) {
                    /* si la requête a fonctionné : mise à jour d'une autre table*/
                    ...
                    if ($result5 == true) {
                        ...
                        if ($result6 == true) {
                            ...
                            if ($result7 == false) {
                                /* alerte si erreur dans la requête */
                                $erreur = true;
        ...
        /* redirection vers le form modifier une édition */
        header('location:modifierEditionInfo.php');
        die();
    }
    ...
    if ($erreur == true) {
        ...
        $_SESSION['editionAModifier'] = $nomEdition;
    }

    /* redirection vers le form modifier édition */
    header('location:modifierEditionLieu.php');
    die();
}
...
/***** CREER UN NOUVEAU LIEU *****/
    ...
    if (isset($_SESSION['edition'])) {
        /* redirection vers le form ajouter un lieu */
        header('location:ajouterLieu.php');
        die();
    } else {
        /* erreur car infos manquantes */
...
/***** AJOUTER UNE PERSONNE MANUELLEMENT *****/
    ...
    if (!isset($_SESSION['edition'])) {
        ...
        if (isset($_POST['nom']) && isset($_POST['prenom']) && isset($_POST['email']) && isset($_POST['telephone']) && isset($_POST['type']) && isset($_POST['disponibilite'])) {
            ...
            // appel à la fonction pour enregistrer la personne dans la BDD et vérifier les erreus
            $erreur = enregistrerPersonneDansBDD($link, $nom, $prenom, $email, $telephone, $informations, $type, $disponibilite, $heuresTravail);
        } else {
            /* erreur si des champs obligatoires ne sont pas remplis */
            $erreur = true;
    ...
    if ($erreur == true) {
        ...
        /* si aucune erreur n'est détectée : suppression des variables de session + alerte */
        unset($_SESSION['nom']);
...
/***** AJOUTER UNE LISTE DE PERSONNE A L'AIDE d4UN CSV *****/

else if (isset($_POST['importerCSV'])) {
    /* mise à false du booléen erreur pour détecter s'il y a ou non une erreur */
    $erreur = false;
    ...
    if (!in_array($fileExtension, $fileExtensionsAllowed)) {
        ...
        /* si le fichier CSV est bien importé */
