...
if (isset($_SESSION['creneauASupprimer'])) {
    ...
    echo "<form action='actionAjouterLieu.php' method='post'>";
...
<div class="arrierePlanRectangle">
    <h1 class="titrePrincipal">ajout d'un lieu</h1>
    <div class="rectangleJaune" style="background-color: <?php echo $couleur4 ?>; color: <?php echo $couleur7 ?>;">

        <form class="formAjoutLieu" action="actionAjouterLieu.php" method="post">
            ...
            <div class="parametreLieu">
                ...
                <?php if (isset($_SESSION['idLieuCree'])) { ?>
                    <input class="boutonSansFond boutonRouge" style='background-color: <?php echo $couleur2 ?>;color: <?php echo $couleur4 ?>; margin-left:8%;' type="submit" name="modifLieu" value="Modifier ce lieu">
                <?php } else { ?>
                    <input class="boutonSansFond boutonRouge" style='background-color: <?php echo $couleur2 ?>;color: <?php echo $couleur4 ?>; margin-left:8%;' type="submit" name="ajoutLieu" value="Ajouter ce lieu">
                <?php } ?>
            </div>


            <span id='nombreDeCreneau' hidden><?php if (isset($_SESSION['creneauCree'])) {
                                                    echo $_SESSION['creneauCree'];
                                                } else {
                                                    echo "-1";
                                                } ?></span>
            <div class="formAjoutCreneau" id="creneau" hidden>

                <?php
                if (isset($_SESSION['nomLieu'])) { ?>
                    ...
                    $requete1 = "SELECT idLieu FROM lieu WHERE nom='" . $_SESSION['nomLieu'] . "'";
                    $result1 = mysqli_query($link, $requete1);
                    if (mysqli_num_rows($result1) == 1) {
                        $row1 = mysqli_fetch_assoc($result1);
                        $requete2 = "SELECT * FROM creneau WHERE idLieu='" . $row1['idLieu'] . "' AND idEdition='" . $_SESSION['edition'] . "'  ORDER BY dateDebut ASC, heureDebut ASC";
                        ...
                        if (mysqli_num_rows($result2) > 0) {
                            ...
                            while ($row2 = mysqli_fetch_assoc($result2)) {
                                if ($nbBenevole != $row2['nbBenevoleManquant'] || $nbMembre != $row2['nbMembreManquant'] || $heureFin != $row2['heureDebut'] || ($date != $row2['dateDebut'] && $row2['heureDebut'] != '00:00:00')) {
                                    if ($date != "00/00/0000") {
                                        if (isset($modifCreneau)) {
                                            if ($idsCreneau == $modifCreneau) {
                                                ...
                                                if (isset($_SESSION['modifHeureFin'])) {
                                                    if ($_SESSION['modifHeureFin'] == "24:00:00") {
                                                        echo "<input class='champInfoCreneau' name='modifHeureFin' type='time' value='00:00:00'>";
                                                    } else {
                                ...
                                if ($nouveauCreneau == true) {
                                    $idsCreneau = $row2['idCreneau'];
                                    $date = $row2['dateDebut'];
                                    $heureDebut = $row2['heureDebut'];
                                    $heure = explode(':', $row2['heureDebut']);
                                    $heureSuivante = intval($heure['0']) + 1;
                                    if ($heureSuivante < 10) {
                                    ...
                                    $nbBenevole = $row2['nbBenevoleManquant'];
                                    $nbMembre = $row2['nbMembreManquant'];
                                    $nouveauCreneau = false;
                                } else {
                                    $idsCreneau = $idsCreneau . "_" . $row2['idCreneau'];
                                    $heure = explode(':', $row2['heureDebut']);
                                    $heureSuivante = intval($heure['0']) + 1;
                                    if ($heureSuivante < 10) {
                                        $heureFin = "0" . $heureSuivante . ":00:00";
...
<!-- JS pour l'apparition du form du créneau -->
<script type="text/javascript">
    let nombreCreneau = document.getElementById("nombreDeCreneau").innerHTML;
    if (nombreCreneau >= 0) {
        document.getElementById("creneau").hidden = false;
    }
</script>
